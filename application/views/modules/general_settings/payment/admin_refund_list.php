                            ...
                            foreach ($refund_data as $refund) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        if ($refund->approved == 0) { ?>
                                            <a title="Approve" href="javascript:void(0);" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" onclick="approve_refund(<?php echo $refund->id; ?>)">
                                                <span class="svg-icon svg-icon-success svg-icon-2hx"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                            ...
                                            <a title="Reject" href="javascript:void(0);" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" onclick="reject_refund(<?php echo $refund->id; ?>)">
                                        ...
                                        <?php } ?>
                                        <a title="View" href="javascript:void(0);" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" onclick="view_refund(<?php echo $refund->id; ?>)">
...
<script>
    function approve_refund(id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: baseurl + 'client/approve-refund',
                    type: 'POST',
                    data: {
                        id: id
                    },
    ...
    function reject_refund(id) {
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: baseurl + 'client/reject-refund',
                    type: 'POST',
                    data: {
                        id: id
                    },
    ...
    function view_refund(id) {
        ...
        var ops_url = baseurl + 'client/view-refund';
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
