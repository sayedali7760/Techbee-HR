                                    ...
                                    <td>
                                        <?php if ($data->status == 'success') { ?>
                                            <?php if ($data->status_finished == 'approved') { ?>
                                                <a title="Process" href="javascript:void(0);"
                                                    ...
                                                    onclick="process_transaction('<?php echo $data->id; ?>')">
                                        ...
                                        <?php } else { ?>
                                            <a title="Approve" href="javascript:void(0);"
                                                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                                                onclick="approve_transaction('<?php echo $data->id; ?>')">
                                            ...
                                            <a title="Reject" href="javascript:void(0);"
                                                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                                                onclick="reject_transaction('<?php echo $data->id; ?>')">
...
<script>
    function approve_transaction(transaction_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var ops_url = baseurl + 'transaction/approve-deposit/';
                $.ajax({
                    type: "POST",
                    cache: false,
                    async: false,
    ...
    function reject_transaction(transaction_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var ops_url = baseurl + 'transaction/reject-deposit/';
                $.ajax({
                    type: "POST",
                    cache: false,
                    async: false,
    ...
    function process_transaction(transaction_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var ops_url = baseurl + 'transaction/process-deposit/';
                $.ajax({
                    type: "POST",
                    cache: false,
                    async: false,
